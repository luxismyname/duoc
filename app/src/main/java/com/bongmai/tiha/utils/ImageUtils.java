...
//            Bitmap bitmapDecode = BitmapFactory.decodeStream(new FileInputStream(f), null, options);
    ...
    public static File getOutputMediaFile(int type) {
        ...
        File mediaStorageDir = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES), "BMTiha");
...
//                        + MainActivity.GALLERY_DIRECTORY_NAME + " directory");
                //return null;
                mediaStorageDir = new File(Environment.getExternalStorageDirectory(), "BMTiha");
                if (!mediaStorageDir.exists()) {
                    if (!mediaStorageDir.mkdirs()) {
                        return null;
                    }
