...
public class LetterTileProvider {
     ...
     */
    private static final int NUM_OF_TILE_COLORS = 13;
//    private static final int NUM_OF_TILE_COLORS = 1;
    ...
    public LetterTileProvider(Context context) {
        ...
        mDefaultBitmap = drawableToBitmap(ContextCompat.getDrawable(context, R.mipmap.ic_launcher));
    ...
    public Bitmap getLetterTileSTTHangBanChay(String displayName) {
        // workaround
        ...
        c.setBitmap(bitmap);
        int COLOR_TOP = -1;
        if (displayName.equals("01")) {
            COLOR_TOP = Color.parseColor("#f16364");
        } else if (displayName.equals("02")) {
            COLOR_TOP = Color.parseColor("#67bf74");
        } else if (displayName.equals("03")) {
            COLOR_TOP = Color.parseColor("#2093cd");
        } else {
            COLOR_TOP = Color.parseColor("#5f8b95");
        }
        c.drawColor(COLOR_TOP);

        char[] mChar = displayName.toCharArray();
        mPaint.setTextSize(mTileLetterFontSize);
        mPaint.getTextBounds(mChar, 0, mChar.length, mBounds);
        c.drawText(mChar, 0, mChar.length, mWidth / 2, mHeight / 2
                + (mBounds.bottom - mBounds.top) / 2, mPaint);
        return bitmap;
    }
...
//        c.drawColor(pickColor(displayName));
        ...
        String color[] = mContext.getResources().getStringArray(R.array.letter_tile_colors);
        try {
            final int positionColor = Math.abs(displayName.hashCode()) % NUM_OF_TILE_COLORS;
            c.drawColor(Color.parseColor(color[positionColor]));
        } catch (Resources.NotFoundException e) {
            c.drawColor(Color.parseColor(color[0]));
        }
